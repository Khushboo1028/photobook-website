...
const OthersImagePage = () => {
  ...
  const flowerPics = imageArray.filter((e) => e.label === "others");
  const listItems = flowerPics.map((item) => (
    <div>
  ...
  return (
    <div>
      <h1 style={{ textAlign: "center" }}>View all Other pictures here!</h1>
      <div style={{ padding: "3rem" }}>{listItems}</div>
    </div>
  );
};

export default OthersImagePage;
